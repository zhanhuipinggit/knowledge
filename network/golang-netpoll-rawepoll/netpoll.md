...
将代码保存到支持 Mermaid 的工具中（例如 [Mermaid Live Editor](https://mermaid-js.github.io/mermaid-live-editor/) 或支持 Mermaid 的 Markdown 编辑器）即可生成类似的模型图。
...
以下代码展示了如何使用 `netpoll` 库实现一个简单的 WebSocket 服务。我们将依赖 `netpoll` 和 `gorilla/websocket` 两个库来完成高效的 WebSocket 连接处理。
