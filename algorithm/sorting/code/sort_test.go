...
// TestInsertSort tests the insertSort function with various input slices.
func TestInsertSort(t *testing.T) {
	testCases := []struct {
		...
		{[]int{9, 8, 7, 6, 5, 4, 3, 2, 1}, []int{1, 2, 3, 4, 5, 6, 7, 8, 9}}, // 测试逆序数组
		{[]int{1, 2, 3, 4, 5, 6, 7, 8, 9}, []int{1, 2, 3, 4, 5, 6, 7, 8, 9}}, // 测试已排序数组
		{[]int{1, 3, 2, 4, 5, 6, 8, 7, 9}, []int{1, 2, 3, 4, 5, 6, 7, 8, 9}}, // 测试部分有序数组
	}
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("Input: %v", tc.input), func(t *testing.T) {
			...
			insertSort(numsCopy)
			if !reflect.DeepEqual(numsCopy, tc.expect) {
				t.Errorf("insertSort(%v) = %v; want %v", tc.input, numsCopy, tc.expect)
			}
		})
	}
}
